			     ...
			     k_timeout_t timeout)
{
	...
	/*
	 * SMP fix: hold _sched_spinlock while iterating the wait queue.
	 * Without this, a concurrent timer ISR on another CPU can call
	 * z_thread_timeout() which acquires _sched_spinlock and removes a
	 * node from rx_msg_queue, corrupting the list under our iterator.
	 *
	 * Lock hierarchy: mbox->lock (already held) → _sched_spinlock.
	 * Use the locked scheduler API variants inside to avoid re-acquiring.
	 */
	       ...
	       k_timeout_t timeout)
{
	...
	/*
	 * SMP fix: same locking requirement as mbox_message_put().
	 * Hold _sched_spinlock while iterating tx_msg_queue so that a
	 * concurrent timeout ISR cannot corrupt the list under us.
	 */
	struct k_thread *matching_thread = NULL;
